...
namespace PowerGridEditor
{
    ...
    internal sealed class ConsoleApplicationEngine
    {
        ...
        // Новые массивы для сохранения дополнительных параметров узлов
        private readonly double[] unom_orig = new double[inn];
        private readonly double[] v_zd = new double[inn];
        private readonly double[] q_min = new double[inn];
        private readonly double[] q_max = new double[inn];
        ...
        private bool Preparation()
        {
            ...
            foreach (var raw in sourceCduLines)
            {
                ...
                if (code == 102 || code == 0102)
                {
                    ...
                    nn[0] = ParseI(t[2]);
                    unom[0] = ParseD(t[3]);
                    unom_orig[0] = unom[0]; // Сохраняем исходное номинальное напряжение
                    p0[0] = ParseD(t[4]);
                    q0[0] = ParseD(t[5]);
                    nk[0] = 3; // Базисный узел
                    g[0] = b[0] = 0;

                    // Считываем уставки для Базисного узла, если они заданы в строке
                    if (t.Length > 8) v_zd[0] = ParseD(t[8]);
                    else v_zd[0] = unom[0];
                    if (t.Length > 9) q_min[0] = ParseD(t[9]);
                    if (t.Length > 10) q_max[0] = ParseD(t[10]);
                ...
                else if (code == 201 || code == 0201)
                {
                    ...
                    nn[j] = ParseI(t[2]);
                    unom[j] = ParseD(t[3]);
                    unom_orig[j] = unom[j]; // Сохраняем исходное номинальное напряжение
                    p0[j] = ParseD(t[4]);
                    q0[j] = ParseD(t[5]);
                    nk[j] = 1; // По умолчанию Нагрузка
                    g[j] = b[j] = 0;
                    ...
                    if (t.Length > 8)
                    {
                        ...
                        if (uv > 0.1)
                        {
                            unom[j] = uv; // Для расчетного движка подменяем unom
                            nk[j] = 2;   // Становится Генераторным узлом
                        }
                    }
                    if (t.Length > 9) q_min[j] = ParseD(t[9]);
                    if (t.Length > 10) q_max[j] = ParseD(t[10]);
        ...
        private string BuildNetworkRez()
        {
            ...
            for (int i = 0; i <= n; i++)
            {
                ...
                // Текстовое определение типа узла
                string typeStr = "Нагр";
                if (nk[i] == 3) typeStr = "База";
        ...
        private string BuildLossesRez()
        {
            ...
            {
                double sia = tzr[j];
                for (int i = 1; i <= L; i++) if (kr[i] == j) sia += tr[i];
                if (Math.Abs(sia) > 1e-10)
